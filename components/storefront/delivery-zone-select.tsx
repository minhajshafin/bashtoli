...
export function DeliveryZoneSelect({
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="text-xs font-bold uppercase tracking-wider text-gold-500 block mb-2">
          Fulfillment Method
        </label>
          ...
          <button
            ...
            className={`flex flex-col items-center justify-center rounded-2xl py-4 border text-center transition-all ${
              fulfillmentType === 'delivery'
                ? 'bg-gold-300/20 border-gold-500 text-forest-900 ring-2 ring-gold-500/20'
                : 'bg-white border-forest-200 text-forest-600 hover:border-gold-500'
            }`}
          >
            <svg className="h-5 w-5 mb-1.5 text-gold-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center rounded-2xl py-3.5 border text-center transition-all ${
                deliveryZone === 'inside_dhaka'
                  ? 'bg-forest-800 border-forest-800 text-cream-100 shadow-md'
                  : 'bg-white border-forest-200 text-forest-700 hover:border-gold-500'
              }`}
            >
      ...
      {fulfillmentType === 'pickup' && (
        <div className="rounded-2xl bg-cream-100 border border-forest-200 p-4 text-xs text-forest-600">
          <p className="font-bold text-forest-900 mb-1">Store Pickup Location</p>
          <p>Bashtoli Showroom, Shajadpur, Bashtola, Gulshan-1212, Dhaka</p>
