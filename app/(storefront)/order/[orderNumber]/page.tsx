...
export async function generateMetadata({ params }: OrderPageProps) {
  ...
  return {
    title: `Order ${resolvedParams.orderNumber} Confirmed | Bashtoli`,
    description: 'Thank you for your purchase from Bashtoli.',
...
export default async function OrderConfirmationPage({ params }: OrderPageProps) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
      {/* Confirmation Success Alert Banner */}
      <div className="flex flex-col items-center text-center mb-10">
        ...
        <p className="mt-2.5 text-sm text-zinc-500 max-w-md dark:text-zinc-400">
          Thank you for shopping at Bashtoli, <span className="font-bold text-zinc-800 dark:text-zinc-200">{order.customer_name}</span>. Your order has been registered and is pending verification.
        </p>
      </div>
      ...
      <div className="bg-white border border-zinc-200 rounded-3xl p-6 md:p-8 shadow-sm dark:bg-zinc-950 dark:border-zinc-800 space-y-6">
            ...
            {items.map((item) => (
              <div key={item.id} className="flex justify-between items-center text-sm">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-zinc-400 mt-0.5">Quantity: {item.qty}</p>
                </div>
                <div className="text-right pl-4">
                  <p className="font-bold text-zinc-900 dark:text-zinc-50">৳{(item.price_at_purchase * item.qty).toLocaleString()}</p>
                  {item.qty > 1 && (
                    <p className="text-[10px] text-zinc-405 mt-0.5">৳{item.price_at_purchase.toLocaleString()} each</p>
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="space-y-3 bg-stone-50/50 p-5 rounded-2xl border border-zinc-150 dark:bg-zinc-900/20 dark:border-zinc-800">
          ...
          <div className="flex justify-between items-baseline">
            <span className="text-sm font-bold text-zinc-900 dark:text-zinc-50">Total Paid (COD)</span>
