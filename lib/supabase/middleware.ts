...
function setSecurityHeaders(res: NextResponse): NextResponse {
  ...
  res.headers.set(
    ...
    [
      ...
      `img-src 'self' data: blob: ${supabaseOrigin} https://images.unsplash.com https://*.unsplash.com https://*.googleapis.com https://maps.gstatic.com https://*.gstatic.com https://*.google.com`,
      `connect-src 'self' ${supabaseOrigin} https://vitals.vercel-insights.com https://va.vercel-scripts.com https://maps.googleapis.com https://*.googleapis.com https://*.google.com`,
...
export async function adminGuard(request: NextRequest) {
  ...
  // Guest order lookup and tracking pages (/order/lookup, /order/[orderNumber])
  const isOrderLookup = pathname.startsWith('/order') || pathname === '/order-lookup'
  if (isOrderLookup) {
